...
func (s *Service) GetFlavorID(flavorID, flavorName *string) (string, error) {
	if flavorID != nil {
		s.scope.Logger().V(4).Info("Resolving flavor by ID", "flavorID", *flavorID)
		return *flavorID, nil
	}
	...
	s.scope.Logger().V(4).Info("Resolving flavor by name", "flavorName", *flavorName)
	...
	s.scope.Logger().V(4).Info("Flavor list retrieved", "count", len(allFlavors))

	flavorNames := make([]string, 0, len(allFlavors))
	for _, flavor := range allFlavors {
		flavorNames = append(flavorNames, flavor.Name)
		if flavor.Name == *flavorName {
			s.scope.Logger().V(4).Info("Flavor resolved", "flavorName", *flavorName, "flavorID", flavor.ID)
			return flavor.ID, nil
		}
	}

	s.scope.Logger().V(4).Info("Flavor not found in list", "requestedName", *flavorName, "availableFlavors", flavorNames)
